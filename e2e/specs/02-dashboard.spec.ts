...
test.describe('Dashboard Page', () => {
  ...
  test('should display device statistics with real data', async ({ page }) => {
    ...
    const stats = await page.locator('.stat-value, mat-card-content, .card-value').allTextContents();
  ...
  test('should display recent devices table or list', async ({ page }) => {
    ...
    const table = page.locator('table, mat-table, .device-list, .devices-table');
    ...
    if (hasList) {
      ...
      const emptyState = page.locator('.empty-state, .no-data, p:has-text("No devices")');
  ...
  test('should navigate to devices page', async ({ page }) => {
    ...
    const devicesLink = page.locator('a:has-text("Devices"), a:has-text("Thiết bị"), a[routerLink="/devices"]').first();
  ...
  test('should refresh data when refresh button clicked', async ({ page }) => {
    ...
    const refreshButton = page.locator('button:has-text("Refresh"), button[aria-label="Refresh"], button mat-icon:has-text("refresh")').first();
    
    if (await refreshButton.isVisible()) {
      // Get initial stat value
      const initialStat = await page.locator('.stat-value, .card-value').first().textContent();
      ...
      const newStat = await page.locator('.stat-value, .card-value').first().textContent();
