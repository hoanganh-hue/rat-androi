...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      else if (filename.includes('camera') || filename.includes('photo')) logType = LogType.CAMERA;
      else if (filename.includes('audio') || filename.includes('recording')) logType = LogType.AUDIO;
      ...
      logger.info(`File uploaded from ${deviceModel}: ${file.originalname} (${file.size} bytes)`);
