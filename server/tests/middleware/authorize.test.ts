...
describe('authorize middleware (real JWT)', () => {
  ...
  it('returns 403 for insufficient role', async () => {
    const app = express();
    app.get('/admin', authenticate, adminOnly, (_req, res) => res.json({ ok: true }));
    const viewer = generateToken({ id: 1, username: 'u', role: 'viewer' });
    const res = await request(app).get('/admin').set('Authorization', `Bearer ${viewer}`);
    expect(res.status).toBe(403);
  });

  it('allows manager for adminOrManager', async () => {
    const app = express();
    app.get('/mgr', authenticate, adminOrManager, (_req, res) => res.json({ ok: true }));
    const mgr = generateToken({ id: 2, username: 'm', role: 'manager' });
    const res = await request(app).get('/mgr').set('Authorization', `Bearer ${mgr}`);
